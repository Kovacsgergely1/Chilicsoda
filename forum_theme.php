<?php
    ...
    if(isset($_POST["send_message"])){
        ...
        if(mysqli_num_rows(mysqli_query($conn, $sql_check_message)) > 0){
            echo "<p class='alert alert-warning'>ugyanazt az üzenetet nem üzenhetet ugyanabban a témában!</p>";
            // !!!
        }
        elseif($filetred_message == ""){
            echo "<p class='alert alert-warning'>üres üzenetet nem küldhetsz!</p>";
            // !!!
        }
        else{
            // üzenet feltöltése
...
<div id="Sidenav" class="sidenav">
    ...
    <form action="forum_theme.php" method="post">
        <br>
        <a href="forum.php" id="return">Vissza</a> 
        <br>
        <?php
        if(isset($_SESSION["username"])){
            echo 'üzenet:';
